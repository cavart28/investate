...
def values_of_series_of_invest(
    ...
    [1.05, 2.05]
    >>> t = values_of_series_of_invest([1, 1], [0.05, 0.08], invest_at_begining_of_period=True, final_only=False)
    >>> assert np.allclose(t, [1.05, 2.214], rtol=1e-01)

    Tt can easily be used to get total invested value after several regular investments
...
def compute_mortg_principal(
    ...
    if loan_rate == 0:
        ...
        invest_value_factor = (period_rate_factor ** n_periods - 1) / (
            period_rate_factor - 1
        )
...
def compute_equity_and_interest(
    ...
    if estate_growth_rate > 0:
        equity_over_time = [
            equ * period_estate_growth_factor ** n_period
...
def house_investment(
    ...
    house_value = [
        house_cost * (1 + estate_rate / 12) ** i for i in range(n_total_months)
    ]
    # if nothing were repaid, the loan would follow this
    unrepaid_loan_remaining = [
        loan_amount * (1 + mortg_rate / 12) ** i for i in range(n_months_repay)
    ]
    ...
    if plot:
        ...
        plt.vlines(
            x=n_months_repay,
            ymin=np.min(monthly_income),
            ymax=np.max(monthly_income),
            label="mortgage paid off",
            linestyles="dashed",
            linewidth=0.5,
        )
...
def compare_house_invest_vs_stock(
    ...
    """Compare real estate and stock investment"""
    ...
    down_payment_invest = [
        down_payment_perc * house_cost * (1 + stock_market_month_rate) ** i
